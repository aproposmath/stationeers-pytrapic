...
namespace StationeersPyTrapIC
{
    ...
    public class FileServerPlugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            F.Log($"File server started on http://localhost:{Port}/ serving {_basePath}");
        }
        ...
        private void HandleRequest(HttpListenerContext context)
        {
            ...
            F.Log($"Serve path {path}");
            if (path.StartsWith("/api/"))
            {
                try
                {
                    F.Log($"API request for {path}");
                    HandleApi(context, path);
                }
                catch (Exception ex)
                {
                    F.Log($"Error handling API request: {ex}");
                }
            }
            else
            {
        ...
        {
            F.Log($"Handling API request for {path}");
            var response = "{}";
            int status = 200;
            ...
            try
            {
                var chip = CursorManager.CursorThing as ProgrammableChip;

                if (path.Equals("/api/upload", StringComparison.OrdinalIgnoreCase))
                {
                    F.Log("Upload request received");
                    // Read request body
                    using (
                    ...
                    {
                        var uploadData = JsonConvert.DeserializeObject<UploadData>(
                            reader.ReadToEnd()
                        );
                        if (chip == null)
                        {
                            throw new InvalidOperationException("No chip found in cursor");
                        }

                        if (uploadData.OldIC10Code != chip.GetSourceCode())
                        {
                            throw new InvalidOperationException(
                                "Old IC10 code does not match current chip code"
                            );
                        }

                        ChipData chipData = new ChipData
                        {
                            PythonCode = uploadData.PythonCode,
                            IC10Code = uploadData.IC10Code,
                        };

                        ChipPatch.ChipsData[chip.ReferenceId] = chipData;
                        chip.SetSourceCode(uploadData.IC10Code);
                        F.Log("Uploaded source code");
                        response = JsonConvert.SerializeObject(
                            new { message = "Source code uploaded successfully" }
                        );
                        var compiledCode = PythonCompiler.Instance.Compile(
                            uploadData.PythonCode,
                            new CompileOptions()
                        );
                        F.Log($"Compiled code: {compiledCode}");
                    }
                }
                else if (path.Equals("/api/get", StringComparison.OrdinalIgnoreCase))
                {
                    F.Log("Request for code");
                    if (chip == null)
                    {
                        return;
                    }
                    ...
                    {
                        PythonCode = "",
                        IC10Code = chip.GetSourceCode(),
                        OldIC10Code = chip.GetSourceCode(),
                    };

                    if (ChipPatch.ChipsData.ContainsKey(chip.ReferenceId))
                    {
                        var chipData = ChipPatch.ChipsData[chip.ReferenceId];
                        data.PythonCode = chipData.PythonCode;
                        data.OldIC10Code = chipData.IC10Code;
                    }
                ...
                {
                    status = 404;
                    response = JsonConvert.SerializeObject(new { message = "Unknown endpoint" });
                }
            }
            catch (Exception ex)
            {
                status = 500;
                F.Log($"Error handling API request: {ex}");
                response = JsonConvert.SerializeObject(new { message = $"error: {ex.Message}" });
            }
            finally
            {
                try
                ...
                {
                    F.Log($"Error writing response: {ex}");
                }
                finally
                {
        ...
        private void ServeFile(HttpListenerContext context, string path)
        {
            ...
            {
                F.Log($"File serving error: {ex}");
                context.Response.StatusCode = 500;
                byte[] data = Encoding.UTF8.GetBytes("500 Internal Server Error");
                context.Response.OutputStream.Write(data, 0, data.Length);
            }
            finally
            {
