...
- `constexpr` function decorator to force evaluation at compile time
- Compile flags
...
def filter_by_name(name, negate=False):
    hash = HASH(name) << 8
    instruction = SorterInstruction.FilterPrefabHashNotEquals if negate else SorterInstruction.FilterPrefabHashEquals
